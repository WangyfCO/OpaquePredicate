...
def deobfu_func(func_addr,length):
    blocks = set()
    ...
    state = proj.factory.blank_state(addr=func_addr,args=[proj.filename,sym_val])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--length', type=lambda x : int(x, 0), help='Determining length of target function')
    ...
    length=args.length
    deobfu_func(func_addr=start,length=length)
    # Write the recovered binary file to ${file}_recovered
    fname, ext = os.path.splitext(args.file)
    with open(fname + '_recovered'+str(length) + ext, 'wb') as file:
        file.write(binfile)
    print('Deobfuscation success!')
    now_finished=datetime.datetime.now().strftime("%Y-%m-%d %H-%M:%S")
    print("finished!",now_finished)
